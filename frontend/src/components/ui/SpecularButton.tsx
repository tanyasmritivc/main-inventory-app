...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    // Capture teardown so WebGL init failures are absorbed silently.
    // The button's CSS border + tint remain fully readable without the effect.
    ...
    try {
      ...
      // WebGL unavailable (Safari with hardware acceleration off, iOS low-power
      // mode, incognito on some browsers). The button's CSS border, tint and text
      // are fully readable without the specular canvas overlay.
    }
      ...
      style={
        {
          ...
          padding: size === 'lg' ? '18px 52px' : size === 'sm' ? '9px 18px' : '14px 30px',
          fontSize: size === 'lg' ? '1.2rem' : size === 'sm' ? '0.85rem' : '1rem',
