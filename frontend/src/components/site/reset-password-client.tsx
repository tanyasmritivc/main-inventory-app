  ...
  return (
    <main style={{ minHeight: "100dvh", display: "grid", placeItems: "center", padding: 20, background: "var(--light-page)", color: "var(--text-primary)" }}>
      <section style={{ width: "min(420px, 100%)", padding: 28, border: "1px solid rgba(58,18,48,.1)", borderRadius: 16, background: "var(--light-panel)", boxShadow: "0 25px 70px rgba(58,18,48,.10)" }}>
        ...
        {checking ? <p style={{ color: "var(--light-muted)", fontSize: 13 }}>Checking your reset link…</p> : null}
        {!checking && (!hasSession || expired) ? (
          <div>
            <p role="alert" style={{ color: "var(--danger-ink)", fontSize: 13, lineHeight: 1.55 }}>This reset link is invalid or has expired. Request a new one from the sign-in page.</p>
            <Link href="/signin" style={{ display: "inline-flex", marginTop: 10, color: "var(--copper)", fontSize: 13 }}>Return to sign in</Link>
          </div>
        ) : null}
