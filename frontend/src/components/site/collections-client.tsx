...
export function CollectionsClient() {
    ...
    return (
      <div style={{ padding: '36px 40px', maxWidth: '700px', fontFamily: FONT, WebkitFontSmoothing: 'antialiased' as any }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: '-0.03em', color: 'var(--text-primary)' }}>Before I Buy</div>
          <div style={{ width: 60 }} />
        </div>
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 1 }}>{it.location}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {dismissalsEnabled ? (
              <div style={{ position: 'relative' }}>
                <button
                  type="button"
                  style={{ fontSize: 11, color: 'var(--text-secondary)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}
                  aria-label="More"
    ...
    return (
      <div style={{ padding: '36px 40px', maxWidth: '900px', fontFamily: FONT, WebkitFontSmoothing: 'antialiased' as any }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, letterSpacing: '-0.03em', color: 'var(--text-primary)' }}>Restock Essentials</div>
          <button
            type="button"
            disabled={loading}
            onClick={async () => {
        ...
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 24, letterSpacing: '-0.01em' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          <div style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 14, backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', boxShadow: '0 1px 3px rgba(0,0,0,0.3), inset 0 1px 0 rgba(0,0,0,0.05)', padding: '20px 22px' }}>
            ...
            {(restockUrgent ?? []).length === 0 ? (
              <div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '16px 0', textAlign: 'center' as any }}>Nothing urgent right now.</div>
            ) : null}
          ...
          <div style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 14, backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', boxShadow: '0 1px 3px rgba(0,0,0,0.3), inset 0 1px 0 rgba(0,0,0,0.05)', padding: '20px 22px' }}>
            ...
            {(restockSoon ?? []).length === 0 ? (
              <div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '16px 0', textAlign: 'center' as any }}>Nothing low right now.</div>
            ) : null}
            {(restockSoon ?? []).map((it) => {
              const removing = !!restockRemoving[it.item_id];
              const menuOpen = !!restockMenuOpen[it.item_id];
        ...
        <div style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 14, backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', boxShadow: '0 1px 3px rgba(0,0,0,0.3), inset 0 1px 0 rgba(0,0,0,0.05)', padding: '20px 22px' }}>
          ...
          {(restockForgotten ?? []).length === 0 ? (
            <div style={{ fontSize: 12, color: 'var(--text-muted)', textAlign: 'center' as any, padding: '16px 0' }}>None flagged.</div>
          ) : null}
  ...
  return (
    <div style={{ padding: '36px 40px', maxWidth: '1100px', fontFamily: FONT, WebkitFontSmoothing: 'antialiased' as any }}>
      {error ? <p style={{ fontSize: 13, color: 'var(--danger-ink)', marginBottom: 16 }}>{error}</p> : null}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 12 }}>
        ...
        >
          ...
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.55, letterSpacing: '-0.01em', flex: 1 }}>Check if you already own something before purchasing.</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 10, letterSpacing: '-0.005em' }}>
        ...
        >
          ...
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.55, letterSpacing: '-0.01em', flex: 1 }}>What do you need right now?</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 10, letterSpacing: '-0.005em' }}>
