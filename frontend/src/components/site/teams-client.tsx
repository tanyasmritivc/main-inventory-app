...
import {
  ...
  itemDisplayDescription, itemDisplayName,
...
export function TeamsClient() {
  ...
  if (!selectedId && !loading) return <section className="product-page"><header className="product-page-header"><h1>Team</h1><div className="product-actions"><button className="product-button" onClick={() => setCreateMode("join")}>Join</button><button className="product-button primary" onClick={() => setCreateMode("team")}><Plus size={15} />New team</button></div></header>{createMode && <TeamCreateForm mode={createMode} form={form} setForm={setForm} onSubmit={submitCreate} onCancel={() => setCreateMode(null)} working={working} />}{error && <div className="notice-error">{error}</div>}<div className="bare-empty"><Users size={24} /><strong>No teams</strong></div></section>;

  return (
    <section className="teams-page product-page">
      <header className="product-page-header team-page-header"><div className="team-title-row"><select className="team-title-select" aria-label="Current team" value={selectedId ?? ""} onChange={(event) => { setSelectedId(event.target.value); setOpenSpace(null); }}>{teams.map((entry) => <option key={entry.team_id} value={entry.team_id}>{entry.name}</option>)}</select><span className="status-pill">{role}</span></div><div className="product-actions"><button className="product-button" onClick={() => setCreateMode("join")}>Join</button><button className="product-button" onClick={() => setCreateMode("team")}><Plus size={15} />New</button><button className="app-icon-button" aria-label="Refresh team" onClick={() => void loadWorkspace()}><RefreshCw size={15} /></button></div></header>
      ...
      {tab === "spaces" && <div className="team-panel"><div className="team-panel-heading"><h2>Spaces</h2>{canEdit && <div className="product-actions">{canManage && <button className="product-button" onClick={() => setCreateMode("space")}><Plus size={14} />Create</button>}<button className="product-button primary" onClick={() => setCreateMode("attach")}><Link2 size={14} />Link</button></div>}</div>{openSpace ? <div className="team-space-detail product-card"><div className="team-space-detail-head"><button className="product-button" onClick={() => setOpenSpace(null)}>← Spaces</button><div><h3>{openSpace.name}</h3><span>{spaceItems.length} items</span></div>{canEdit && <button className="product-button primary" onClick={() => setCreateMode("item")}><Plus size={14} />Add</button>}</div><div className="team-item-table"><div className="team-item-head"><span>Item</span><span>Category</span><span>Qty</span><span /></div>{spaceItems.map((item) => <div className="team-item-row" key={item.item_id}><span><strong>{itemDisplayName(item)}</strong><small>{itemDisplayDescription(item) || item.brand || ""}</small></span><span>{item.category}</span><span>{item.quantity}</span><span>{canEdit && <><button onClick={() => void editSpaceItem(item)}>Edit</button><button onClick={() => void removeSpaceItem(item)}>Delete</button></>}</span></div>)}</div></div> : <div className="team-space-grid">{spaces.map((space) => <article key={space.id}><button onClick={() => void loadSpace(space)}><Archive size={18} /><span><strong>{space.name}</strong><small>{space.item_count ?? 0} items{space.owned_by_me ? " · yours" : ""}</small></span></button>{(canManage || space.owned_by_me) && <button className="team-card-menu" onClick={() => void removeSpace(space)} aria-label={`Remove ${space.name}`}><Trash2 size={14} /></button>}</article>)}{spaces.length === 0 && <div className="bare-empty"><Archive size={22} /><strong>No shared Spaces</strong></div>}</div>}</div>}

      {tab === "board" && <div className="team-panel"><div className="team-panel-heading"><h2>Board</h2>{canEdit && <button className="product-button primary" onClick={() => setCreateMode("task")}><Plus size={14} />New</button>}</div><div className="board-columns">{(["todo", "doing", "done"] as const).map((status) => <section key={status}><header><span>{status === "todo" ? "To do" : status === "doing" ? "In progress" : "Done"}</span><small>{board.filter((task) => task.status === status).length}</small></header><div>{board.filter((task) => task.status === status).map((task) => <article className="board-card product-card" key={task.task_id}><div className="board-card-top"><span className={`status-pill ${task.priority === "urgent" ? "red" : task.priority === "high" ? "amber" : ""}`}>{task.task_type.replace("_", " ")}</span>{canEdit && <button onClick={() => void removeTask(task)}><MoreHorizontal size={15} /></button>}</div><h3>{task.title}</h3>{task.description && <p>{task.description}</p>}<footer><span>{task.assigned_to ? memberName(memberMap.get(task.assigned_to)) : "Unassigned"}</span>{canEdit && <select value={task.status} onChange={(event) => void changeTask(task, { status: event.target.value as TeamBoardTask["status"] })}><option value="todo">To do</option><option value="doing">In progress</option><option value="done">Done</option></select>}</footer></article>)}</div></section>)}</div></div>}

      {tab === "people" && <div className="team-panel"><div className="team-panel-heading"><h2>People</h2>{canManage && <div className="product-actions"><button className="product-button" onClick={() => void copyInvite()}><Clipboard size={14} />Copy link</button><button className="product-button primary" onClick={() => void sendInvite()}><Send size={14} />Invite</button></div>}</div>{canManage && team?.join_code && <div className="invite-code product-card"><span>Join code</span><strong>{team.join_code}</strong><button className="product-button" onClick={() => void rotateCode()}>Reset</button></div>}<div className="people-list">{members.map((member) => <article key={member.user_id}><div className="member-avatar" style={{ background: member.avatar_color || "#3a3a3c" }}>{memberName(member)[0]?.toUpperCase()}</div><div><strong>{memberName(member)}</strong><span>{[member.profile_role, member.organization, member.contact_email].filter(Boolean).join(" · ")}</span></div>{canManage && member.role !== "owner" ? <select value={member.role} onChange={(event) => void updateRole(member, event.target.value as TeamRole)}><option value="mentor">Manager</option><option value="member">Member</option><option value="viewer">Viewer</option></select> : <span className="status-pill">{member.role}</span>}{canManage && member.role !== "owner" && <button className="app-icon-button" onClick={() => void removeMember(member)} aria-label={`Remove ${memberName(member)}`}><UserMinus size={15} /></button>}</article>)}</div><button className="product-button danger team-exit" onClick={() => void exitTeam()}>{role === "owner" ? "Delete team" : "Leave team"}</button></div>}

      {tab === "files" && <div className="team-panel"><div className="team-panel-heading"><h2>Files</h2>{canEdit && <><button className="product-button primary" onClick={() => teamFileRef.current?.click()}><Plus size={14} />Upload</button><input ref={teamFileRef} hidden type="file" onChange={(event) => void uploadDocument(event)} /></>}</div><div className="team-files">{documents.map((document) => <article key={document.team_document_id}><FileText size={18} /><button onClick={() => void openDocument(document)}><strong>{document.filename}</strong><span>{relativeTime(document.created_at)} · {document.size_bytes ? `${Math.ceil(document.size_bytes / 1024)} KB` : "File"}</span></button>{canEdit && <button className="app-icon-button" onClick={() => void removeDocument(document)}><Trash2 size={14} /></button>}</article>)}{documents.length === 0 && <div className="bare-empty"><FileText size={22} /><strong>No files</strong></div>}</div></div>}

      {tab === "activity" && <div className="team-panel"><div className="team-panel-heading"><h2>Activity</h2></div><div className="activity-feed">{activity.map((entry) => <article className="activity-row" key={entry.activity_id}><span className="activity-icon"><History size={15} /></span><div><div className="activity-copy">{entry.summary}</div><div className="activity-meta">{relativeTime(entry.created_at)}</div></div></article>)}{activity.length === 0 && <div className="bare-empty"><History size={22} /><strong>No activity</strong></div>}</div></div>}
    </section>
  );
}
