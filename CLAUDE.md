...
- **Production SMTP is configured.** On 2026-08-30 GoTrue was moved to Brevo SMTP relay on port
  587 with STARTTLS, using verified sender `FindEZ <noreply@findez.ai>`. The credential exists only
  in the production Supabase `.env`; it is not in this repository or the iPhone app. The Auth
...
enforce read scopes independently. The public reference is at `/docs/api`, linked
from Settings → API keys and the landing-page footer. Its downloadable OpenAPI
file is generated by `scripts/api_reference.py`; `tests/api_docs` checks the
contract and HTTP examples, and the PostgreSQL CI job checks database semantics.
...
**FIND inventory-photo pipeline (2026-09-19):** Both Flutter and Next.js send
...
solid `#171717` cards, `#1C1C1E` elevated surfaces, brand blue `#0066B3` interaction accents,
...
Project screens to solid `#171717` surfaces and the shared `#0066B3` interaction accent. Chat's explicit SF Pro
...
restrained borders, brand blue only for interactive emphasis, and semantic red/amber/green only for real states.
...
review. FindEZ uses one brand blue (`#0066B3`) only for primary actions, selected controls,
links, and coachmarks. Cards and page identities stay black/graphite/white with no blue tint, glow,
or gradient. Success remains green, attention remains amber, and destructive actions remain red.
The brand-blue correction passed `flutter analyze`; a configured release build was installed
and launched on Tanya's physical iPhone on 2026-08-30.
...
real operation label with indeterminate progress; primary save is a solid brand blue control and
...
labels and brand blue only for interactive emphasis. Settings remains the home of support, legal,
...
The interaction accent was then changed system-wide from teal to brand blue `#0066B3`.
Hardcoded copies across all active mobile surfaces and the legacy multicolor primary gradient were
replaced so the app has one interaction color. Green, amber, and red remain semantic state colors.
The approved supporting palette is slate `#417B9B`, ice `#C2DAF4`, indigo `#343078`, lavender
...
orientation, tutorial targeting, new-chat behavior, join-space access, and Settings navigation.
It passed `flutter analyze` on 2026-08-30.
